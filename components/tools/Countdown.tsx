...
export default function Countdown() {
  ...
  const cell = (n: number, label: string) => (
    <div className="bg-rose-50 rounded-xl py-4 flex-1">
      ...
      <div className="text-xs text-rose-400 mt-0.5">{label}</div>
    </div>
  )
  ...
  return (
    <div className="bg-white rounded-3xl border border-rose-100 shadow-sm p-6 sm:p-8">
      ...
      {valid && !passed && (
        <div className="mt-6 border-t border-rose-50 pt-6 flex gap-2.5 sm:gap-3 text-center">
          {cell(days, 'days')}
          {cell(hours, 'hours')}
          {cell(mins, 'minutes')}
          {cell(secs, 'seconds')}
      ...
      {passed && (
        <div className="mt-6 border-t border-rose-50 pt-6 text-center">
          ...
          <p className="font-serif text-xl text-rose-900">The day is here!</p>
